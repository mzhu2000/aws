...
> Getting started with the Terraform for managing a base mzhu2000 AWS resources.
...
5. Create config file `./src/mzhu2000/backend/config.tf` that will contain information how to store state in a given bucket. See [example](./src/mzhu2000/backend/example.config.tf).
6. Create SSH key pair to connect to EC2 instance:
   ```bash
   cd ./src/mzhu2000/provision/access

   # it creates "mzhu2000-ec2-key" private key and "mzhu2000-ec2-key.pub" public key
   ssh-keygen -f mzhu2000-ec2-key
   ``` 
...
1. `cd ./src/mzhu2000`
2. `ssh -i ./provision/access/mzhu2000-ec2-key ubuntu@[EC2 public IP]`
